...
function openDetail(id){const m=state.meals.find(x=>x.id===id);if(!m)return;const ingr=typeof m.ingredients==='string'?`<p>${m.ingredients}</p>`:MODULES.map(k=>`<p><b>${k}:</b> ${(m.ingredients[k]||[]).join(', ')||'-'}</p>`).join('');dlg.innerHTML=`<h2>${m.name}</h2><img src='${mealPhoto(m)}' alt='${m.name} photo'><p class='small'>${m.date}</p><p style='color:${ratingColor(m.rating)}'>Rating ${m.rating}/10</p><p>${m.note||''}</p>${ingr}<button id='edit'>Edit</button> <button id='del'>Delete</button> <button id='close'>Close</button>`;dlg.showModal();dlg.querySelector('#close').onclick=()=>dlg.close();dlg.querySelector('#edit').onclick=()=>{editingId=id;view='add';dlg.close();render();};dlg.querySelector('#del').onclick=async()=>{if(confirm('Delete meal?')){state.meals=state.meals.filter(x=>x.id!==id);await saveState();dlg.close();render();}}}
