...
app.innerHTML=`<div class='meal-grid'>${meals.map(m=>`<article class='card' data-id='${m.id}'><img src='${m.photo||""}' alt='meal photo'><div class='card-body'><h3>${m.name}</h3><span class='rating-pill' style='background:${ratingColor(m.rating)}'>${m.rating}/10</span></div></article>`).join('')||'<p>No meals yet.</p>'}</div>`;
...
function openDetail(id){const m=state.meals.find(x=>x.id===id);if(!m)return;const ingr=typeof m.ingredients==='string'?`<p>${m.ingredients}</p>`:MODULES.map(k=>`<p><b>${k}:</b> ${(m.ingredients[k]||[]).join(', ')||'-'}</p>`).join('');dlg.innerHTML=`<h2>${m.name}</h2><img src='${m.photo||''}'><p class='small'>${m.date}</p><p style='color:${ratingColor(m.rating)}'>Rating ${m.rating}/10</p><p>${m.note||''}</p>${ingr}<button id='edit'>Edit</button> <button id='del'>Delete</button> <button id='close'>Close</button>`;dlg.showModal();dlg.querySelector('#close').onclick=()=>dlg.close();dlg.querySelector('#edit').onclick=()=>{editingId=id;view='add';dlg.close();render();};dlg.querySelector('#del').onclick=async()=>{if(confirm('Delete meal?')){state.meals=state.meals.filter(x=>x.id!==id);await saveState();dlg.close();render();}}}
function renderShopping(){app.innerHTML=`<div class='form-section'><h3>Plan a meal</h3><select id='mealPick'><option value=''>Select meal</option>${state.meals.map(m=>`<option value='${m.id}'>${m.name}</option>`)}</select><input id='dayPick' placeholder='Day (e.g. Monday)'><button id='planAdd' type='button'>Add planned meal</button></div>
...
${state.shoppingList.map((g,gi)=>`<div class='shopping-group'><h4>📅 ${g.day}</h4><p>🍽️ ${g.meal}</p>${g.items.map((it,ii)=>`<label class='${it.checked?'line-through':''}'><input type='checkbox' data-gi='${gi}' data-ii='${ii}' ${it.checked?'checked':''}> ${it.name}</label><br>`).join('')}</div>`).join('')}
...
function renderSettings(){app.innerHTML=`<section class='form-section'><h3>Ingredient Library</h3>${MODULES.map(m=>`<div><b>${m}</b> ${(state.ingredientLibrary[m]||[]).map((i,idx)=>`<span class='chip'>${i} <button data-del='${m}:${idx}'>x</button></span>`).join('')}<div><input id='add-${m}' placeholder='Add ${m}'><button data-addm='${m}'>Add</button></div></div>`).join('')}</section>
<section class='form-section'><h3>Data</h3><button id='export'>Export JSON</button><input id='import' type='file' accept='application/json'></section>
<section class='form-section'><h3>About</h3><p>Version 1.0.0</p><p class='small'>Offline-first personal cooking journal PWA.</p></section>`;
